...
public class Table{
	...
	public static String[] getColName(String table){ //tables=davisbase_tables
		...
		try{
			...
			String[] attr = {"rowid", "table_name", "column_name", "data_type", "ordinal_position", "is_nullable"};
			String[] userargs = {"table_name","=",table};
			filter(file, userargs, attr, buffer);
			HashMap<Integer, String[]> record = buffer.record;
			ArrayList<String> array = new ArrayList<String>();
			for(String[] i : record.values()){
				array.add(i[2]);
			}
			int size=array.size();
			attrs = array.toArray(new String[size]);
			file.close();
			return attrs;
		}catch(Exception e){
			System.out.println(e);
	...
	public static void filter(RandomAccessFile file, String[] userargs, String[] attr, Buffer buffer){
		try{
			...
			for(int page = 1; page <= numOfPages; page++){
				...
				if(pageType == 0x0D)
				{
					...
					for(int i=0; i < numOfCells; i++){
						...
						boolean check = userargsCheck(vals, rowid, userargs, attr);
						
						if(check)
							buffer.insert_records(rowid, vals);
					}
				}
				else
					continue;
			...
			buffer.attr = attr;
			buffer.Qtype = new int[attr.length];
		...
		try{
			...
			int num_attrs = file.readByte();
			
			byte[] stc = new byte[num_attrs];
			file.read(stc);
			
			values = new String[num_attrs+1];
...
public static int searchKeyPage(RandomAccessFile file, int key){
	...
	public static void filter(RandomAccessFile file, String[] userargs, String[] attr, String[] type, Buffer buffer){
		try{
	...
	public static boolean userargsCheck(String[] values, int rowid, String[] userargs, String[] attr){
		...
		if(userargs.length == 0){
			...
			if(colPos == 1){
				int val = Integer.parseInt(userargs[2]);
				String operator = userargs[1];
				switch(operator){
					case "=": if(rowid == val) 
								check = true;
							  else
				...
				if(userargs[2].equals(values[colPos-1]))
					check = true;
				else
					check = false;
			}
